...
function shouldNotifyForChatSnapshot(chat, prevSnapshot, nextSnapshot) {
    ...
    const prevUnread = Number((prevSnapshot && prevSnapshot.unread) || 0);
...
function getActiveTypingTarget() {
    if (window.channelsModule && typeof window.channelsModule.currentChannel === 'function') {
        const channel = window.channelsModule.currentChannel();
        if (window.channelsModule.isChannelActive && window.channelsModule.isChannelActive() && channel && channel.id) {
            return { chat_type: 'channel', chat_id: channel.id };
        }
    }
    if (window.groupsModule && typeof window.groupsModule.currentGroup === 'function') {
        const group = window.groupsModule.currentGroup();
        if (window.groupsModule.isGroupActive && window.groupsModule.isGroupActive() && group && group.id) {
...
window.setCurrentChat = function(chat) { currentChat = chat; };
window.getCurrentChat = function() { return currentChat; };

// Отправка сообщения в топик
async function sendMessageToTopic(topicId, content, ttlSeconds = 0) {
    const token = localStorage.getItem('xipher_token');
    if (!token) {
    ...
    try {
        ...
        if (data.success) {
            ...
            if (window.loadAndRenderTopicMessages) {
                await window.loadAndRenderTopicMessages(topicId);
            }
        } else {
            notifications.error(data.error || 'Ошибка отправки сообщения');
        }
...
window.sendMessageToTopic = sendMessageToTopic;
...
function normalizeBusinessHours(raw) {
    ...
    Object.keys(BUSINESS_HOURS_DEFAULTS).forEach(day => {
        ...
        normalized[day] = {
            enabled: value && typeof value.enabled === 'boolean' ? value.enabled : defaults.enabled,
            start: value && typeof value.start === 'string' ? value.start : defaults.start,
            end: value && typeof value.end === 'string' ? value.end : defaults.end
        };
    });
    return normalized;
}
...
function createUnreadDivider() {
    ...
    divider.innerHTML = `
        ...
        <span class="divider-text">Непрочитанные сообщения</span>
        <div class="divider-line"></div>
    `;
    return divider;
}
...
function updatePremiumGiftAvailability() {
    const giftBtn = document.getElementById('attachMenuPremiumGift');
    if (!giftBtn) return;
    const channelActive = window.channelsModule && typeof window.channelsModule.isChannelActive === 'function' ? window.channelsModule.isChannelActive() : false;
    const groupActive = window.groupsModule && typeof window.groupsModule.isGroupActive === 'function' ? window.groupsModule.isGroupActive() : false;
...
function renderChats() {
    ...
    const { pinned, unread, read } = sortChatsForDisplay(visibleChats);
    const totalChats = pinned.length + unread.length + read.length;
    console.log('Rendering', totalChats, 'chats (pinned:', pinned.length, ', unread:', unread.length, ', read:', read.length + ')');
    ...
    // Add divider if there are both unread and read chats (excluding pinned)
    if (unread.length > 0 && read.length > 0) {
        fragment.appendChild(createUnreadDivider());
    }
...
function getActiveChatSelection() {
    const activeChannel = window.channelsModule && typeof window.channelsModule.currentChannel === 'function' ? window.channelsModule.currentChannel() : null;
    if (activeChannel && activeChannel.id) {
        return { id: activeChannel.id, type: 'channel' };
    }
    const activeGroup = window.groupsModule && typeof window.groupsModule.currentGroup === 'function' ? window.groupsModule.currentGroup() : null;
...
function restoreLastChatSelection() {
    ...
    if (stored.type === 'group') {
        if (match.data && window.groupsModule && typeof window.groupsModule.selectGroup === 'function') {
            window.groupsModule.selectGroup(match.data);
        }
        return;
    }
    if (stored.type === 'channel') {
        if (match.data && window.channelsModule && typeof window.channelsModule.selectChannel === 'function') {
...
function getChatDeepLinkTarget() {
    ...
    const params = new URLSearchParams(window.location.search);
    const chatId = (match && match[1]) || params.get('chat_id') || params.get('chatId') || params.get('chat') || '';
...
function createChatItem(chat) {
    ...
    item.addEventListener('click', async (e) => {
        ...
        if (chat.type === 'group' && window.groupsModule) {
            const groupData = chat.data || { id: chat.id, name: chat.name };
            console.log('[Chat] Selecting group with data:', groupData);
            window.groupsModule.selectGroup(groupData);
        } else if (chat.type === 'channel') {
...
function selectChat(chat) {
    ...
    // Hide topic tabs when switching to regular chat
    ...
    // Add/update send money button for regular chats
    let sendMoneyBtn = document.getElementById('sendMoneyBtn');
    if (chat.is_saved_messages || chat.is_bot || chat.type === 'group' || chat.type === 'channel') {
        if (sendMoneyBtn) sendMoneyBtn.style.display = 'none';
    } else {
        if (!sendMoneyBtn) {
            sendMoneyBtn = document.createElement('button');
            sendMoneyBtn.id = 'sendMoneyBtn';
            sendMoneyBtn.className = 'chat-action-btn';
            sendMoneyBtn.title = 'Отправить деньги';
            sendMoneyBtn.innerHTML = '💸';
            sendMoneyBtn.style.background = 'linear-gradient(135deg, #10b981, #059669)';
            sendMoneyBtn.style.color = '#fff';
            sendMoneyBtn.onclick = () => {
                if (typeof XipherWalletUI !== 'undefined') {
                    XipherWalletUI.open();
                    setTimeout(() => {
                        XipherWalletUI.showSend('@' + (chat.username || chat.display_name || ''));
                    }, 100);
                }
            };
            const chatActions = document.querySelector('.chat-actions');
            if (chatActions && callBtn) {
                chatActions.insertBefore(sendMoneyBtn, callBtn);
            }
        }
...
async function sendMessage() {
    ...
    // Check for money transfer pattern: @username $amount
    if (typeof XipherWallet !== 'undefined' && XipherWallet.hasWallet()) {
        const moneyTransfer = XipherWallet.parseMoneyTransfer(message);
        if (moneyTransfer) {
            // Show wallet send modal with pre-filled data
            if (typeof XipherWalletUI !== 'undefined') {
                XipherWalletUI.open();
                setTimeout(() => {
                    XipherWalletUI.showSend('@' + moneyTransfer.recipient, moneyTransfer.amount);
                }, 100);
            }
            return;
        }
    ...
    // Проверяем, активен ли топик (форум)
    if (window.currentTopicId && window.currentTopic) {
        console.log('[Chat] Sending message to topic:', window.currentTopicId);
        await sendMessageToTopic(window.currentTopicId, outgoingContent, ttlSeconds);
        input.value = '';
        input.style.height = 'auto';
        replyToMessage = null;
        const replyPreview = document.getElementById('replyPreview');
    ...
    const isGroupActive = window.groupsModule && typeof window.groupsModule.isGroupActive === 'function' 
        ? window.groupsModule.isGroupActive() 
        : false;
    ...
    if (window.groupsModule && (isGroupActive || activeGroup)) {
...
// =========================
// Profile modal (Xipher style)
// =========================

async function openUserProfile(params) {
    console.log('[openUserProfile] Called with params:', params);
    
    // Prevent opening user profile for groups/channels
    if (window.currentGroup || (window.groupsModule && window.groupsModule.isGroupActive && window.groupsModule.isGroupActive())) {
        console.log('[openUserProfile] Blocked - group is active, opening group settings instead');
        if (window.groupsModule && typeof window.groupsModule.openGroupSettings === 'function') {
    ...
    if (window.channelsModule && window.channelsModule.isChannelActive && window.channelsModule.isChannelActive()) {
        console.log('[openUserProfile] Blocked - channel is active, opening channel info instead');
    ...
    // Inject Xipher-style profile CSS
    if (!document.getElementById('xipherProfileStyles')) {
        const styleEl = document.createElement('style');
        styleEl.id = 'xipherProfileStyles';
        styleEl.textContent = `
            .xipher-profile-overlay {
                position: fixed;
                inset: 0;
            ...
            .xipher-profile-close:hover {
                background: var(--purple-primary);
                color: #fff;
                border-color: var(--purple-primary);
            }
            .xipher-profile-avatar {
                width: 90px;
                height: 90px;
                border-radius: 50%;
            ...
            .xipher-profile-avatar img {
                width: 100%;
                height: 100%;
                object-fit: cover;
            }
            .xipher-profile-avatar .online-dot {
                position: absolute;
                bottom: 4px;
                right: 4px;
                width: 16px;
            ...
            .xipher-profile-status {
                font-size: 14px;
                color: var(--success-color);
                margin-bottom: 16px;
            }
            .xipher-profile-status.offline {
                color: var(--text-muted);
            }
            .xipher-profile-actions {
                display: flex;
                justify-content: center;
                gap: 20px;
            }
            .xipher-profile-action {
                display: flex;
                flex-direction: column;
                align-items: center;
                gap: 6px;
            ...
            .xipher-profile-action:hover {
                color: var(--text-primary);
            }
            .xipher-profile-action:hover .action-icon {
                background: var(--purple-primary);
                color: #fff;
                transform: scale(1.05);
            }
            .xipher-profile-action .action-icon {
                width: 44px;
                height: 44px;
                border-radius: 50%;
                background: var(--bg-tertiary);
            ...
            .xipher-profile-action .action-label {
                font-size: 12px;
                font-weight: 500;
            }
            .xipher-profile-body {
            ...
            .xipher-profile-section {
                margin-bottom: 12px;
            }
            .xipher-profile-section-title {
                font-size: 12px;
                font-weight: 600;
            ...
            .xipher-profile-row:hover {
                border-color: var(--border-color);
                background: var(--bg-secondary);
            }
            .xipher-profile-row-icon {
                font-size: 20px;
                color: var(--purple-primary);
                flex-shrink: 0;
                width: 24px;
            ...
            .xipher-profile-row-content {
                flex: 1;
                min-width: 0;
            }
            .xipher-profile-row-value {
                font-size: 15px;
                color: var(--text-primary);
                word-break: break-word;
            }
            .xipher-profile-row-label {
                font-size: 13px;
                color: var(--text-muted);
                margin-top: 2px;
            }
            .xipher-profile-stats {
                display: grid;
            ...
            .xipher-profile-stat:hover {
                border-color: var(--border-color);
                background: var(--bg-secondary);
            }
            .xipher-profile-stat-icon {
                font-size: 16px;
                color: var(--text-muted);
            }
            .xipher-profile-stat-text {
                font-size: 14px;
                color: var(--text-secondary);
            }
            .xipher-profile-stat-count {
                font-weight: 600;
                color: var(--text-primary);
                margin-left: auto;
            }
            .xipher-profile-groups {
                display: flex;
                flex-direction: column;
                gap: 6px;
            }
            .xipher-profile-group {
                display: flex;
                align-items: center;
                gap: 10px;
                padding: 10px 12px;
            ...
            .xipher-profile-group:hover {
                border-color: var(--border-color);
                background: var(--bg-secondary);
            }
            .xipher-profile-group-avatar {
                width: 36px;
                height: 36px;
                border-radius: 50%;
                background: var(--gradient-purple);
            ...
            .xipher-profile-group-avatar img {
                width: 100%;
                height: 100%;
                object-fit: cover;
            }
            .xipher-profile-group-name {
                font-size: 14px;
                color: var(--text-primary);
            }
            .xipher-profile-divider {
                height: 1px;
                background: var(--border-color);
                margin: 8px 0;
            }
            .xipher-profile-footer {
                padding: 8px 0;
            }
            .xipher-profile-btn {
                display: flex;
            ...
            .xipher-profile-btn:hover {
                background: var(--bg-tertiary);
                color: var(--text-primary);
            }
            .xipher-profile-btn.danger {
                color: var(--error-color);
            }
            .xipher-profile-btn.danger:hover {
                background: rgba(239, 68, 68, 0.1);
            }
            .xipher-profile-btn-icon {
                font-size: 18px;
                width: 24px;
                text-align: center;
            }
            ...
            .xipher-profile-hours {
                background: var(--bg-secondary);
                border-radius: 10px;
                padding: 10px;
                margin-top: 8px;
            }
            .xipher-profile-hours-header {
                display: flex;
                align-items: center;
                gap: 8px;
                margin-bottom: 8px;
            }
            .xipher-profile-hours-badge {
                padding: 3px 8px;
                border-radius: 8px;
                font-size: 11px;
                font-weight: 600;
            }
            .xipher-profile-hours-badge.open {
                background: rgba(16, 185, 129, 0.2);
                color: var(--success-color);
            }
            .xipher-profile-hours-badge.closed {
                background: rgba(239, 68, 68, 0.2);
                color: var(--error-color);
            }
            .xipher-profile-hours-hint {
                font-size: 12px;
                color: var(--text-muted);
            }
            .xipher-profile-hours-row {
                display: flex;
                justify-content: space-between;
                font-size: 13px;
                padding: 3px 0;
            }
            .xipher-profile-hours-day {
                color: var(--text-muted);
            }
            .xipher-profile-hours-day.today {
                color: var(--purple-primary);
                font-weight: 600;
            }
            .xipher-profile-hours-time {
                color: var(--text-secondary);
            }
            .xipher-profile-hours-time.off {
                color: var(--text-muted);
            }
        `;
    ...
    const overlay = document.createElement('div');
    overlay.className = 'xipher-profile-overlay';
    overlay.id = 'userProfileModal';
    overlay.innerHTML = `
        <div class="xipher-profile-modal">
            <div class="xipher-profile-header">
                <button class="xipher-profile-close" id="closeUserProfileModal">×</button>
                <div class="xipher-profile-avatar"><span>...</span></div>
                <h2 class="xipher-profile-name">Загрузка...</h2>
                <div class="xipher-profile-status offline">загрузка...</div>
            </div>
            <div class="xipher-profile-body">
                <div style="text-align:center; padding:30px; color:var(--text-muted);">Загрузка профиля...</div>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
    ...
    try {
        ...
        const onlineText = isOnline ? 'в сети' : (u.last_seen ? `был(а) ${u.last_seen}` : 'был(а) недавно');
        const stats = u.media_stats || {};
        const commonGroups = u.common_groups || [];

        const birth = (u.birth_day && u.birth_month)
            ? `${u.birth_day} ${getMonthName(u.birth_month)}`
            : '';
        const personalChannel = u.personal_channel && u.personal_channel.id ? u.personal_channel : null;
        
        // Business hours
        let businessHoursHtml = '';
        if (u.business_hours && typeof u.business_hours === 'object' && Object.keys(u.business_hours).length > 0) {
            const bh = u.business_hours;
            const days = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
            const dayNames = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'];
            const today = new Date().getDay();
            const todayIndex = today === 0 ? 6 : today - 1;
            const { isOpen, nextOpenInfo } = getBusinessHoursStatus(bh);
            
            let hoursRows = '';
            days.forEach((day, idx) => {
                const schedule = bh[day];
                ...
                hoursRows += `<div class="xipher-profile-hours-row">
                    <span class="xipher-profile-hours-day${isToday ? ' today' : ''}">${dayLabel}</span>
                    <span class="xipher-profile-hours-time${isOff ? ' off' : ''}">${timeStr}</span>
                </div>`;
            });
            
            businessHoursHtml = `
                <div class="xipher-profile-hours">
                    <div class="xipher-profile-hours-header">
                        <span class="xipher-profile-hours-badge ${isOpen ? 'open' : 'closed'}">${isOpen ? 'Открыто' : 'Закрыто'}</span>
                        ${nextOpenInfo ? `<span class="xipher-profile-hours-hint">${escapeHtml(nextOpenInfo)}</span>` : ''}
                    </div>
                    ${hoursRows}
                </div>
            `;
        ...
        // Stats grid - only show items with count > 0
        const statsItems = [
            { key: 'photos', icon: '🖼️', label: 'фотографий' },
            { key: 'videos', icon: '🎬', label: 'видео' },
            { key: 'files', icon: '📄', label: 'файлов' },
            { key: 'audio', icon: '🎵', label: 'аудиофайлов' },
            { key: 'links', icon: '🔗', label: 'ссылок' },
            { key: 'voice', icon: '🎤', label: 'голосовых' },
            { key: 'gifs', icon: '🎞️', label: 'GIF' },
        ];
        
        // Check if user is viewing own profile - show saved
        if (u.is_self && stats.saved > 0) {
            statsItems.push({ key: 'saved', icon: '⭐', label: 'в избранном' });
        }

        let statsHtml = '';
        const visibleStats = statsItems.filter(s => (stats[s.key] || 0) > 0);
        if (visibleStats.length > 0) {
            statsHtml = `
                <div class="xipher-profile-section">
                    <div class="xipher-profile-section-title">Медиа в чате</div>
                    <div class="xipher-profile-stats">
                        ${visibleStats.map(s => `
                            <div class="xipher-profile-stat" data-type="${s.key}">
                                <span class="xipher-profile-stat-icon">${s.icon}</span>
                                <span class="xipher-profile-stat-text">${stats[s.key]} ${s.label}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
        ...
        // Common groups
        let groupsHtml = '';
        if (commonGroups.length > 0 && !u.is_self) {
            groupsHtml = `
                <div class="xipher-profile-section">
                    <div class="xipher-profile-section-title">${commonGroups.length} общих групп</div>
                    <div class="xipher-profile-groups">
                        ${commonGroups.map(g => `
                            <div class="xipher-profile-group" data-group-id="${escapeHtml(g.id)}">
                                <div class="xipher-profile-group-avatar">
                                    ${g.avatar_url ? `<img src="${escapeHtml(g.avatar_url)}" onerror="this.remove()">` : ''}
                                    <span>${escapeHtml((g.name || 'G').charAt(0).toUpperCase())}</span>
                                </div>
                                <span class="xipher-profile-group-name">${escapeHtml(g.name)}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
        ...
        // Render modal
        const modal = overlay.querySelector('.xipher-profile-modal');
        modal.innerHTML = `
            <div class="xipher-profile-header">
                <button class="xipher-profile-close" id="closeUserProfileModal">×</button>
                <div class="xipher-profile-avatar">
                    ${avatarUrl ? `<img src="${avatarUrl}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">` : ''}
                    <span ${avatarUrl ? 'style="display:none;"' : ''}>${escapeHtml(initials)}</span>
                    ${isOnline ? '<div class="online-dot"></div>' : ''}
                </div>
                <h2 class="xipher-profile-name">
                    ${escapeHtml(u.display_name || u.username)}
                    ${u.is_premium ? '<span class="premium-star" title="Premium">★</span>' : ''}
                </h2>
                <div class="xipher-profile-status${isOnline ? '' : ' offline'}">${escapeHtml(onlineText)}</div>
                <div class="xipher-profile-actions">
                    <button class="xipher-profile-action" id="profileMsgBtn" title="Написать">
                        <span class="action-icon">💬</span>
                        <span class="action-label">Чат</span>
                    </button>
                    <button class="xipher-profile-action" id="profileMuteBtn" title="Уведомления">
                        <span class="action-icon">🔔</span>
                        <span class="action-label">Звук</span>
                    </button>
                    <button class="xipher-profile-action" id="profileCallBtn" title="Позвонить">
                        <span class="action-icon">📞</span>
                        <span class="action-label">Звонок</span>
                    </button>
                    <button class="xipher-profile-action" id="profileMoreBtn" title="Ещё">
                        <span class="action-icon">⋯</span>
                        <span class="action-label">Ещё</span>
                    </button>
                </div>
            </div>
            <div class="xipher-profile-body">
                ...
                <div class="xipher-profile-section">
                    ${u.bio ? `
                        <div class="xipher-profile-row" data-copy="${escapeHtml(u.bio)}">
                            <span class="xipher-profile-row-icon">ℹ️</span>
                            <div class="xipher-profile-row-content">
                                <div class="xipher-profile-row-value">${escapeHtml(u.bio)}</div>
                                <div class="xipher-profile-row-label">О себе</div>
                            </div>
                        </div>
                    ` : ''}
                    
                    <div class="xipher-profile-row" data-copy="@${escapeHtml(u.username || '')}">
                        <span class="xipher-profile-row-icon">👤</span>
                        <div class="xipher-profile-row-content">
                            <div class="xipher-profile-row-value">@${escapeHtml(u.username || '')}</div>
                            <div class="xipher-profile-row-label">Имя пользователя</div>
                        </div>
                    </div>
                    
                    ${birth ? `
                        <div class="xipher-profile-row">
                            <span class="xipher-profile-row-icon">🎂</span>
                            <div class="xipher-profile-row-content">
                                <div class="xipher-profile-row-value">${escapeHtml(birth)}</div>
                                <div class="xipher-profile-row-label">День рождения</div>
                            </div>
                        </div>
                    ` : ''}
                    
                    ${businessHoursHtml ? `
                        <div class="xipher-profile-row" style="cursor:default;">
                            <span class="xipher-profile-row-icon">🕐</span>
                            <div class="xipher-profile-row-content">
                                <div class="xipher-profile-row-label" style="margin:0 0 4px;">Часы работы</div>
                                ${businessHoursHtml}
                            </div>
                        </div>
                    ` : ''}
                    ...
                    ${personalChannel ? `
                        <div class="xipher-profile-row" id="profileChannelRow">
                            <span class="xipher-profile-row-icon">📢</span>
                            <div class="xipher-profile-row-content">
                                <div class="xipher-profile-row-value">${escapeHtml(personalChannel.name || 'Канал')}</div>
                                <div class="xipher-profile-row-label">Персональный канал${personalChannel.custom_link ? ` · @${personalChannel.custom_link}` : ''}</div>
                            </div>
                        </div>
                    ` : ''}
                </div>
                
                ${statsHtml ? '<div class="xipher-profile-divider"></div>' : ''}
                ${statsHtml}
                
                ${groupsHtml ? '<div class="xipher-profile-divider"></div>' : ''}
                ${groupsHtml}
                
                <div class="xipher-profile-divider"></div>
                
                <!-- Footer actions -->
                <div class="xipher-profile-footer">
                    ${!u.is_self ? `
                        <button class="xipher-profile-btn" id="profileAddFriendBtn">
                            <span class="xipher-profile-btn-icon">👥</span>
                            <span>${u.is_friend ? '✓ В друзьях' : 'Добавить в друзья'}</span>
                        </button>
                    ` : ''}
                    
                    <button class="xipher-profile-btn" id="profileShareBtn">
                        <span class="xipher-profile-btn-icon">↗️</span>
                        <span>Поделиться контактом</span>
                    </button>
                    
                    ${!u.is_self ? `
                        <button class="xipher-profile-btn danger" id="profileBlockBtn">
                            <span class="xipher-profile-btn-icon">🚫</span>
        ...
        // Chat button
        modal.querySelector('#profileMsgBtn')?.addEventListener('click', () => {
            close();
            if (u.username) {
        ...
        // Call button
        modal.querySelector('#profileCallBtn')?.addEventListener('click', () => {
            if (u.id && typeof window.startCall === 'function') {
                close();
                window.startCall(u.id, 'audio');
            } else if (u.id && typeof window.callsModule !== 'undefined' && typeof window.callsModule.startCall === 'function') {
                close();
                window.callsModule.startCall(u.id, 'audio');
            } else {
                // Fallback: открыть чат и предложить позвонить
                close();
                if (u.username) {
                    openChatWithUser(u.username, !!u.is_friend, u.id, u.is_premium);
                    setTimeout(() => {
                        notifications.info('Звонки доступны через кнопку 📞 в шапке чата');
                    }, 500);
                }
            }
        });

        // Mute button - toggle notifications for this user's chat
        const muteBtn = modal.querySelector('#profileMuteBtn');
        if (muteBtn && u.id) {
            // Check current mute state
            const chatKey = buildChatKeyFromParts(u.id, 'user');
            const isMuted = mutedChatKeys.has(chatKey);
            
            // Update button appearance
            const updateMuteBtn = (muted) => {
                const icon = muteBtn.querySelector('.action-icon');
                const label = muteBtn.querySelector('.action-label');
                if (icon) icon.textContent = muted ? '🔕' : '🔔';
                if (label) label.textContent = muted ? 'Вкл. звук' : 'Звук';
                muteBtn.title = muted ? 'Включить уведомления' : 'Отключить уведомления';
            };
            
            updateMuteBtn(isMuted);
            
            muteBtn.addEventListener('click', () => {
                const currentKey = buildChatKeyFromParts(u.id, 'user');
                const currentlyMuted = mutedChatKeys.has(currentKey);
                const newMuted = !currentlyMuted;
                
                // Toggle mute state
                if (newMuted) {
                    mutedChatKeys.add(currentKey);
                } else {
                    mutedChatKeys.delete(currentKey);
                }
                writeChatKeySet(MUTED_CHATS_KEY, mutedChatKeys);
                
                updateMuteBtn(newMuted);
                notifications.success(newMuted ? 'Уведомления отключены' : 'Уведомления включены');
        ...
        // More button - показать меню с дополнительными действиями
        modal.querySelector('#profileMoreBtn')?.addEventListener('click', (e) => {
            showProfileMoreMenu(e, u, close);
        });

        // Add friend button
        modal.querySelector('#profileAddFriendBtn')?.addEventListener('click', async (e) => {
            const btn = e.currentTarget;
            if (u.is_friend) {
                notifications.info('Уже в друзьях');
                return;
            }
            if (!u.username) return;
            try {
                await sendFriendRequest(u.username);
                btn.innerHTML = '<span class="xipher-profile-btn-icon">✅</span><span>Запрос отправлен</span>';
            } catch (_) {}
        });

        // Share button
        modal.querySelector('#profileShareBtn')?.addEventListener('click', async () => {
        ...
        // Block button
        modal.querySelector('#profileBlockBtn')?.addEventListener('click', async () => {
            if (!u.id) return;
            const confirmed = confirm(`Заблокировать ${u.display_name || u.username}?`);
            if (!confirmed) return;
            
            try {
                const blockRes = await fetch(API_BASE + '/api/block-user', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ token, user_id: u.id })
                });
                ...
                if (blockData.success) {
                    ...
                    notifications.error(blockData.error || 'Ошибка блокировки');
                }
            } catch (err) {
                notifications.error('Ошибка блокировки');
            }
        });

        // Channel row click
        modal.querySelector('#profileChannelRow')?.addEventListener('click', () => {
            if (personalChannel && personalChannel.id) {
                close();
                if (typeof openChannelById === 'function') {
                    openChannelById(personalChannel.id);
                } else if (window.channelsModule && typeof window.channelsModule.openChannel === 'function') {
                    window.channelsModule.openChannel(personalChannel.id);
                }
            }
        });

        // Copy on click for rows with data-copy
        modal.querySelectorAll('.xipher-profile-row[data-copy]').forEach(row => {
        ...
        // Click on common groups
        modal.querySelectorAll('.xipher-profile-group[data-group-id]').forEach(groupEl => {
            groupEl.addEventListener('click', () => {
                const groupId = groupEl.dataset.groupId;
                if (groupId) {
                    close();
                    if (window.groupsModule && typeof window.groupsModule.openGroup === 'function') {
                        window.groupsModule.openGroup(groupId);
                    }
                }
            });
        });
...
// Helper: get month name in Russian
...
// Helper: get business hours status
function getBusinessHoursStatus(bh) {
    if (!bh) return { isOpen: false, nextOpenInfo: '' };
    ...
    if (schedule && schedule.enabled && schedule.open && schedule.close) {
        ...
        if (currentTime >= openTime && currentTime < closeTime) {
            ...
            return { 
                isOpen: true, 
                nextOpenInfo: hours > 0 ? `закроется через ${hours}ч ${mins}м` : `закроется через ${mins}м`
            };
        }
    }
    ...
    // Find next open time
    for (let i = 0; i < 7; i++) {
        const checkDayIdx = (day + i) % 7;
        const checkDay = days[checkDayIdx];
        const checkSchedule = bh[checkDay];
        
        if (checkSchedule && checkSchedule.enabled && checkSchedule.open) {
            if (i === 0) {
                const [openH, openM] = checkSchedule.open.split(':').map(Number);
                const openTime = openH * 60 + openM;
                if (currentTime < openTime) {
                    ...
                    return { 
                        isOpen: false, 
                        nextOpenInfo: hours > 0 ? `откроется через ${hours}ч ${mins}м` : `откроется через ${mins}м`
                    };
                }
            } else {
                return { 
                    isOpen: false, 
                    nextOpenInfo: `откроется в ${dayNames[checkDayIdx]} ${checkSchedule.open}`
                };
            }
        }
    }
    
    return { isOpen: false, nextOpenInfo: '' };
}

window.openUserProfile = openUserProfile;

// Profile "More" menu
function showProfileMoreMenu(event, user, closeProfileFn) {
    // Remove existing menu
    const existingMenu = document.querySelector('.profile-more-menu');
    ...
    menu.innerHTML = `
        <style>
            ...
            .profile-more-menu-item-icon {
                font-size: 16px;
                width: 20px;
                text-align: center;
            }
            .profile-more-menu-divider {
                height: 1px;
                background: var(--border-color);
                margin: 4px 0;
            }
        </style>
        <div class="profile-more-menu-item" data-action="copy-id">
            <span class="profile-more-menu-item-icon">🆔</span>
            <span>Копировать ID</span>
        </div>
        <div class="profile-more-menu-item" data-action="copy-username">
            <span class="profile-more-menu-item-icon">@</span>
            <span>Копировать @username</span>
        </div>
        ${user.is_friend ? `
            <div class="profile-more-menu-divider"></div>
            <div class="profile-more-menu-item" data-action="remove-friend">
                <span class="profile-more-menu-item-icon">👤</span>
        ...
        ${!user.is_self ? `
            <div class="profile-more-menu-divider"></div>
            <div class="profile-more-menu-item" data-action="clear-history">
                <span class="profile-more-menu-item-icon">🗑️</span>
                <span>Очистить историю</span>
            </div>
            <div class="profile-more-menu-item danger" data-action="report">
                <span class="profile-more-menu-item-icon">⚠️</span>
    ...
    // Position menu
    const btnRect = event.target.closest('.xipher-profile-action').getBoundingClientRect();
    ...
    // Handle menu actions
    menu.querySelectorAll('.profile-more-menu-item').forEach(item => {
            ...
            switch (action) {
                ...
                case 'copy-username':
                    try {
                        ...
                        notifications.success('Username скопирован');
                    } catch (_) {}
                    break;
                    
                case 'remove-friend':
                    if (confirm(`Удалить ${user.display_name || user.username} из друзей?`)) {
                        try {
                            const token = localStorage.getItem('xipher_token');
                            const res = await fetch(API_BASE + '/api/remove-friend', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ token, friend_id: user.id })
                            });
                            const data = await res.json();
                            if (data.success) {
                ...
                case 'clear-history':
                    if (confirm(`Очистить историю чата с ${user.display_name || user.username}?`)) {
                        try {
                            const token = localStorage.getItem('xipher_token');
                            const res = await fetch(API_BASE + '/api/clear-chat-history', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ token, peer_id: user.id })
                            });
                            const data = await res.json();
                            if (data.success) {
                                notifications.success('История очищена');
                                closeProfileFn();
                                // Reload messages if this chat is active
                                if (window.currentChatId === user.id) {
                                    loadMessages(user.id);
                                }
                            } else {
                                notifications.error(data.error || 'Ошибка очистки');
                            }
                        } catch (_) {
                            notifications.error('Ошибка очистки истории');
                        }
                    }
                    break;
                ...
                case 'report':
                    ...
                    if (reason && reason.trim()) {
                        try {
                            const token = localStorage.getItem('xipher_token');
                            await fetch(API_BASE + '/api/report-user', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ token, user_id: user.id, reason: reason.trim() })
                            });
                            notifications.success('Жалоба отправлена');
                        } catch (_) {
                            notifications.error('Ошибка отправки жалобы');
                        }
                    }
                    break;
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (headerInfo) {
        ...
        headerInfo.addEventListener('click', () => {
            try {
                console.log('[HeaderClick] Click detected', {
                    channelActive: window.channelsModule && window.channelsModule.isChannelActive && window.channelsModule.isChannelActive(),
                    groupsModuleActive: window.groupsModule && typeof window.groupsModule.isGroupActive === 'function' && window.groupsModule.isGroupActive(),
                    windowCurrentGroup: window.currentGroup,
                    currentChat: currentChat,
                    currentChatType: currentChat ? currentChat.type : 'no chat'
                });
                ...
                // Check both groupsModule.isGroupActive and window.currentGroup for robustness
                const groupActive = (window.groupsModule && typeof window.groupsModule.isGroupActive === 'function' && window.groupsModule.isGroupActive()) || window.currentGroup;
                if (groupActive) {
                    console.log('[HeaderClick] Opening group settings');
                ...
                // Only open user profile for direct chats (not groups/channels)
                if (currentChat && currentChat.id && typeof window.openUserProfile === 'function') {
                    // Skip if this is a group or channel based on currentChat type
                    const chatType = currentChat.type || 'chat';
                    console.log('[HeaderClick] Chat type check:', chatType);
                    
                    if (chatType === 'group') {
                        console.log('[HeaderClick] Opening group settings for group type');
                        if (window.groupsModule && typeof window.groupsModule.openGroupSettings === 'function') {
                            window.groupsModule.openGroupSettings();
                        }
                        return;
                    ...
                    if (chatType === 'channel') {
                        console.log('[HeaderClick] Opening channel info for channel type');
                        if (window.channelsModule && typeof window.channelsModule.openChannelInfoPanel === 'function') {
                            window.channelsModule.openChannelInfoPanel();
                        }
                        return;
                    }
                    ...
                    console.log('[HeaderClick] Opening user profile for chat type:', chatType);
                    window.openUserProfile({ user_id: currentChat.id });
                }
            } catch (err) {
                console.error('[HeaderClick] Error:', err);
            }
        });
    }
});
